...
uint8 DeviationGet(void)
{
  ...
  {
    if(CenterLine[row] != NullValue)
    { //指定范围内找到了有效行.
      tempDevi = CenterLine[row] - PhyCenterCol;
...
void SportControl(uint8 ImageProFlag)
{
  uint8 tempD;
    ...
    VirtualOutData[2] = CenterLine[25];   //BlackRightLoc[25][0]; //FTM1_C0V - ServoPIDMin;
