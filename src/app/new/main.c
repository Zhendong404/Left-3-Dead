...
uint8  Image_pro_flag = 0;      //add for transfer,mxl,2016.3.25
  ...
  while(1)
  {
    ...
    *********************/
    if(TIME0flag_20ms == 1)
    {
      TIME0flag_20ms = 0 ;                                                      //清除时间标志位    
       ...
       Sum_Station = 0;
      TurnPWM_Value = TurnPWM(Station, MIDSTATION) ;                        //根据camera位置值计算转向PWM值函数
    ...
    *********************/
    ...
    //  PTA16_OUT=1;
      ...
      Image_pro_flag = ImagePro();	//用这个flag来判断图像处理是否已经完成，与20ms部分相对应。
