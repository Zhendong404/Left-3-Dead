...
uint8 BlackGetLeftStep1Edge(uint8 row, uint8 use, uint8 CrossFlag)
{
  ...
  temp = CameraRealWidth/2 - 1 + BlackLeftEdgeStartColOffset;	//中值+向右偏移预留量
  index = 0;         //可疑点的序号

  for(col = temp; col >= 4+BlackLeftEdgeNum; col--) 	//4是图像边缘宽度？
  {
...
uint8 BlackGetLeftStep2Win(uint8 row, uint8 win, uint8 predict)
{
  ...
  //对左边界的限制
  if(predict <= win)	//如果预测跳变点不足以向左开窗
  {
    temp = 4 + BlackLeftEdgeNum;	//极限左边缘+最小跳变跨过点数
  }  
  else
  {
    temp = predict - win;
    if(temp < 4 + BlackLeftEdgeNum) temp = 4 + BlackLeftEdgeNum;
  }
  
  //对右边界的限制
  if(predict + win > CameraRealWidth - 1)	//如果预测点向右开窗超过图像右边界
  {
...
uint8 BlackGetLeftStep1Scan(uint8 FirstLine, uint8 LastLine, uint8 CrossFlag)
{
  ...
  //最近的BlackStep1Size行用差值法找黑线
    for(row = FirstLine; row > LastLine; row -= 2)	//从下往上找
    {
      //判断是否连续两行检测到跳变。
...
uint8 BlackLeftAgainForCross(uint8 Row2FirstLine, uint8 Row2LastLine, uint8 Row1RealLastLine)
{
    ...
    if(temp == MaxValUint8)  
    {
      //在预计区间内找不到有效的行，则将之前已记录的数据全部置位无效
    ...
    //temp为预测的黑点位置
    temp = BlackLeftLoc[row-1][0] + BlackLeftLoc[row-1][0] - BlackLeftLoc[row][0];	//？这里有一点问题
    //从最近的认证行到取最远的行
    for(row -= 2; ; row -= 2)
    {
...
//temp1~Step4StartLine为合理取点范围
  index = 0;
  for(i = Step4StartLine; ; i--)	//向上取点
  {
    ...
    if(index == BlackBackLineNum) break;
    if(i == temp1) return 0;
  }
  //最小二乘法
  //y = a0 + a1 * x
  ...
  for(row = Step4StartLine + 1; row <= CameraHight - 1; row++)	//向下补线
  {
    //补线也有个度，向左向右都不能补出范围。
    temp = a0 + a1 * row;
    ...
    else BlackLeftLoc[row][0] = (uint8)(temp);
  }
  return 1;
}
...
uint8 BlackGetLeft(void)
{
    ...
    if(temp == MaxValUint8) return 0;	//没找到    
    else 
    {
      row = temp;
      //起始线头的位置非常高，情况如下，进入下述判断
	  //			------------------------
	  //			|			  /	  \				|
	  //			|			 /	   \			|
	  //			|			/		\			|
    ...
    for(row -= 2; ; row -= 2)
    {
      ...
      else if(temp3)
      {
        ...
        {
          //再检测时的行太远了，就没必要检测了。
          if(row < BlackLeftCrossTooLowLine) break;
          if(!BlackLeftAgainForCross(row - 2, 1, Row1RealLastLine)) BlackLeftCrossDone = 1;  //十字检测失败           
          else BlackLeftCrossDone = 2;  //十字检测成功
...
//======================================================================
//函数名：BoundaryLimitRe(int16 x, unit8 left, uint8 right)
//功  能：范围限制函数
//参  数：位置x，左边界left，右边界right
//返  回：范围内的一个最近值
//影  响：
//说  明：
//======================================================================
...
uint8 CenterLineGet(void)
{
  ...
  if((BlackLeftDone == 1) && (BlackRightDone ==  1))
  {
    ...
    for(row = CameraHight - 1; ; row--)
    {
      ...
      else if((templ == MaxValUint8) && (tempr != MaxValUint8))
      {
        ...
        { //前一行的中心线有效。
          ...
          {
            CenterLineLoc[row] =  BoundaryLimitRe(tempr + CenterLineLoc[row+1] - BlackRightLoc[row+1][0], 0, CameraRealWidth - 1);
          }
          //前一行的中心线无效。
          ...
          { //行数够低才处理，否则不考虑。
            ...
            { //找符合要求的行。
              for(i = 2; i <= 4; i++)
              {
                ...
                {
                   CenterLineLoc[row] = BoundaryLimitRe(tempr + CenterLineLoc[row+i] - BlackRightLoc[row+i][0], 0, CameraRealWidth - 1); 
                   break;
                }
              }
              //没有找到符合要求的行。
      ...
      else if((templ != MaxValUint8) && (tempr == MaxValUint8))
      {
        ...
        {  //上一行的中心线有效。
          if(CenterLineLoc[row+1] != MaxValUint8)
          {
            CenterLineLoc[row] = BoundaryLimitRe(templ + CenterLineLoc[row + 1] - BlackLeftLoc[row+1][0], 0, CameraRealWidth - 1);
          }
          //上一行的中心线无效。
          //如果是近处的行，那就有可能是270度左右黑线错位的情况。
          else if(row > CameraHight - 3)
          ...
          { //找符合条件的行
            if(row <= CameraHight - 5)
            {
              for(i = 2; i <= 4; i++)
              {
                ...
                { //找到了符合条件的行。
                  CenterLineLoc[row] = BoundaryLimitRe(templ + CenterLineLoc[row+i] - BlackLeftLoc[row+i][0], 0, CameraRealWidth - 1);
                  break;
                }
              }
              if(i == 5) CenterLineLoc[row] = MaxValUint8; ////置无效值
    ...
    if(CenterLineLoc[row] == MaxValUint8) 
    {
      FailCount++;
      //对无效值的再处理。
      tempmax = MinRe(BlackLeftHeadLine, BlackRightHeadLine);	//最小线头
    ...
    if(FailCount >= CameraHight - 1) return 0;	//中线采集失败
    ...
    if(row == 0) //
    {
      ...
      { //最近的3行找线头。
        ...
        {
          CenterLocStore = (BlackRightLoc[j][0] - BlackLeftLoc[j][0]) / 2;	//center local store 中心校正
          break;
        }
        if(j <= CameraHight - 4)
        {
...
void ImgPut(void)
{
    ...
    {
      k = 0;
      for(j = temp1; j < temp2; j++)// copy the picture
      {
