...
//                    66ms\src\Sources\C\Function_C\Ftm.c                     /
//    Command line =  "D:\IAR_TEST\Sirius\Sirius V2.3.5                       /
//                    66ms\src\Sources\C\Function_C\Ftm.c" -D IAR -D          /
...
// D:\IAR_TEST\Sirius\Sirius V2.3.5  66ms\src\Sources\C\Function_C\Ftm.c
//    1 /******************************************************************************/
//    2 /*******************************************************************************
//    3   文件名：FTM模块C文件Ftm.c
...
//   56   FTM0_SC = 0x08;//???not enable the interrupt mask
        LDR.N    R0,??DataTable2_9  ;; 0x40038000
        MOVS     R1,#+8
        STR      R1,[R0, #+0]
...
//   85   FTM0_OUTMASK=0X0F;   //0 Channel output is not masked. It continues to operate normally.
        LDR.N    R0,??DataTable2_11  ;; 0x40038060
        MOVS     R1,#+15
        STR      R1,[R0, #+0]
...
//   88   FTM0_COMBINE=0;      //Function for Linked Channels (FTMx_COMBINE)
        LDR.N    R0,??DataTable2_12  ;; 0x40038064
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//   89   FTM0_OUTINIT=0;
        LDR.N    R0,??DataTable2_13  ;; 0x4003805c
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//   90   FTM0_EXTTRIG=0;      //FTM External Trigger (FTMx_EXTTRIG)
        LDR.N    R0,??DataTable2_14  ;; 0x4003806c
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//   91   FTM0_POL=0;          //Channels Polarity (FTMx_POL)
        LDR.N    R0,??DataTable2_15  ;; 0x40038070
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
...
//  100   FTM0_INVCTRL=0;     //反转控制
        LDR.N    R0,??DataTable2_17  ;; 0x40038090
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  101   FTM0_SWOCTRL=0;     //软件输出控制F TM Software Output Control (FTMx_SWOCTRL)
        LDR.N    R0,??DataTable2_18  ;; 0x40038094
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  102   FTM0_PWMLOAD=0;     //FTM PWM Load
        LDR.N    R0,??DataTable2_19  ;; 0x40038098
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  103                       //BIT9:
...
//  106   FTM0_CNTIN=0;       //Counter Initial Value
        LDR.N    R0,??DataTable2_20  ;; 0x4003804c
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  107   FTM0_MOD=7500-1;      //Modulo value,The EPWM period is determined by (MOD - CNTIN + 0x0001)
        LDR.N    R0,??DataTable2_21  ;; 0x40038008
        MOVW     R1,#+7499
        STR      R1,[R0, #+0]
//  108                       //采用龙丘时钟初始化函数，可以得到4分频的频率，系统60M频率时，PWM频率是15M,以此类推
//  109                       //PMW频率=X系统频率/4/(2^FTM1_SC_PS)/FTM1_MOD
//  110   FTM0_C4V=2000;         //设置 the pulse width(duty cycle) is determined by (CnV - CNTIN).
        LDR.N    R0,??DataTable2_22  ;; 0x40038030
        MOV      R1,#+2000
        STR      R1,[R0, #+0]
//  111   FTM0_C5V=0;
        LDR.N    R0,??DataTable2_23  ;; 0x40038038
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  112   FTM0_C6V=2000;
        LDR.N    R0,??DataTable2_24  ;; 0x40038040
        MOV      R1,#+2000
        STR      R1,[R0, #+0]
//  113   FTM0_C7V=0;             ///此处CnV写0是为了使占空比为0，暂时不输出PWM波。   --HJZ
        LDR.N    R0,??DataTable2_25  ;; 0x40038048
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  114   FTM0_CNT=0;         //只有低16位可用    
        LDR.N    R0,??DataTable2_26  ;; 0x40038004
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  115                      
//  116 }
...
QuadInit:
        LDR.N    R0,??DataTable2_27  ;; 0x4004a000
        MOV      R1,#+1536
        STR      R1,[R0, #+0]
//  131   PORTB_PCR1 = PORT_PCR_MUX(6);
        LDR.N    R0,??DataTable2_28  ;; 0x4004a004
        MOV      R1,#+1536
        STR      R1,[R0, #+0]
//  132   PORTB_PCR18 = PORT_PCR_MUX(6);
        LDR.N    R0,??DataTable2_29  ;; 0x4004a048
        MOV      R1,#+1536
        STR      R1,[R0, #+0]
//  133   PORTB_PCR19 = PORT_PCR_MUX(6);
        LDR.N    R0,??DataTable2_30  ;; 0x4004a04c
        MOV      R1,#+1536
        STR      R1,[R0, #+0]
//  134   /*使能FTM1、FTM2时钟*/
...
//  138   FTM1_MOD = 20000;                 //可根据需要设置
        LDR.N    R0,??DataTable2_32  ;; 0x40039008
        MOVW     R1,#+20000
        STR      R1,[R0, #+0]
//  139   FTM2_MOD = 20000;
        LDR.N    R0,??DataTable2_33  ;; 0x400b8008
        MOVW     R1,#+20000
        STR      R1,[R0, #+0]
//  140 
//  141   FTM1_CNTIN = 0;
        LDR.N    R0,??DataTable2_34  ;; 0x4003904c
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  142   FTM2_CNTIN = 0;
        LDR.N    R0,??DataTable2_35  ;; 0x400b804c
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  143 
...
//  235   PORTA_PCR12 = PORT_PCR_MUX(0x3)| PORT_PCR_DSE_MASK;
        LDR.N    R0,??DataTable2_43  ;; 0x40049030
        MOV      R1,#+832
        STR      R1,[R0, #+0]
//  236 
//  237   //使能FTM1模块门控时钟
...
//  251   FTM1_COMBINE = 0;                          //用于已连接通道,全部禁止
        LDR.N    R0,??DataTable2_46  ;; 0x40039064
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  252   FTM1_OUTINIT = 0;                          //外部输出初始化为0
        LDR.N    R0,??DataTable2_47  ;; 0x4003905c
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  253   FTM1_EXTTRIG = 0;                          //外部触发设置，全部禁止
        LDR.N    R0,??DataTable2_48  ;; 0x4003906c
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  254   FTM1_POL = 0;                              //输出极性设置，全部为正极性
        LDR.N    R0,??DataTable2_49  ;; 0x40039070
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  255 
...
//  258   FTM1_INVCTRL = 0;     //FTM倒置功能禁止
        LDR.N    R0,??DataTable2_50  ;; 0x40039090
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  259   FTM1_SWOCTRL = 0;     //软件输出控制禁止
        LDR.N    R0,??DataTable2_51  ;; 0x40039094
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  260   FTM1_PWMLOAD = 0;     //PWM加载控制禁止
        LDR.N    R0,??DataTable2_52  ;; 0x40039098
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  261 
...
//  268   FTM1_C0V = 1828;      //脉宽设置为CnV - CNTIN，占空比为20%
        LDR.N    R0,??DataTable2_53  ;; 0x40039010
        MOVW     R1,#+1828
        STR      R1,[R0, #+0]
...
//  278   //9.3% 2180为上限，右转。
//  279   //
//  280   FTM1_CNT = 0;           //FTM计数器寄存器，低16位可以用，写入任何值将使其回到CNTIN
        LDR.N    R0,??DataTable2_54  ;; 0x40039004
        MOVS     R1,#+0
        STR      R1,[R0, #+0]
//  281 }
        BX       LR               ;; return
